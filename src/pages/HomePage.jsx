...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const data = await fetch(RANDOM_USERS_API_URL)
      const { results } = await data.json()
      setUsers(results)

      const newData = await fetch(POSTS_API_URL)
      const postsData = await newData.json()
      ...
      <div className="home-container">
        <div className="insta-header">
          ...
          <div className="menu">
            ...
            <img
              alt="pratham82.sh's profile picture"
              src={`https://www.github.com/pratham82.png?size=200`}
        ...
        <div className="user-stories">
          {users.length ? (
            users.map(user => {
              ...
              return (
                <div className="userCard" key={uuid}>
                  <div className="avatar-wrapper">
                    <img src={medium} alt="" className="avatar" />
                  </div>
                  <span style={{ paddingTop: '4px', fontSize: '12px' }}>
                    {username.length > 10
                      ? username.slice(0, 8) + '...'
                      : username}
                  </span>
            ...
            users.map(user => {
              ...
              return (
                <div className="post" key={uuid}>
                  ...
                  >
                    <img src={medium} alt="" className="avatar-small" />
                    &nbsp;
                    <span>{username}</span>
                  </div>
                  <img
